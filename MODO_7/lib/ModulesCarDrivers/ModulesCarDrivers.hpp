...
{
public:
  void DebugValuesBluetooth(float RefVelA, float RefVelB, float moduloDist, float RefAng, float deltax, float deltay, float x, float y, float phi, int estadoM);
  void recibeDatosBluetooth(float *referenciaA, float *referenciaB, bool *powerON, Controlador &controlA, Controlador &controlB, movimientoCoche &motorA, movimientoCoche &motorB);
};
