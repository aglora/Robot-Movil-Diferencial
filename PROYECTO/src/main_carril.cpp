...
bool powerON = false;
bool flagCompruebaObstaculo = true;
int ang_barrido_D = 0;
float medida_ant = 0;
...
void loop()
{
  ...
  case linea_recta:
      ...
      RefVelA += deltaVel / 2;
      RefVelB += -deltaVel / 2;
      //la condición de salida de este estado se encuentra en la interrupción del echo del ultrasonidos frontal
    break;
...
void ISR_recogeEchoF()
{
  ...
  //Serial.print("DistF:  ");
  //Serial.println(sonicFrontal.valorMedida);

  if ((sonicFrontal.valorMedida <= distancia_prudente) && flagCompruebaObstaculo && powerON)
  {
    Serial.println("If de rutina");
    Orientacion.resetPID();
    RefVelA = 0;
    RefVelB = 0;
