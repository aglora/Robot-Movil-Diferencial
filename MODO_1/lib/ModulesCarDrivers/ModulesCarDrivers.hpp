...
public:                                                                  // Métodos
  ...
  float tomaMedidaUltrasonidos(void);                                    // Subrutina de medición de distancia
};
...
{
public:
  void DebugValuesBluetooth(unsigned int Ref_Distancia, float Medida_Ultrasonidos, unsigned char PWM);
  void recibeDatosBluetooth(unsigned int *referencia, bool *powerON, Controlador &controlNear, Controlador &controlFar, movimientoCoche &motorA, movimientoCoche &motorB);
};
