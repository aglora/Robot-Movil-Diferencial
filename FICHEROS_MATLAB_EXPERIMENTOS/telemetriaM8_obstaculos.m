...
function telemetriaM8_obstaculos(archivo)
close all;
tel=load(archivo);
muestras=length(tel);
...
hold on
plot([0 2.5],[0 0],'-','LineWidth',2); %Trayectoria sin obstaculos
plot([0],[0],'rx','LineWidth',5,'MarkerSize',30); %Punto inicial
plot([2.5],[0],'bx','LineWidth',5,'MarkerSize',30); %Punto final
...
legend('Trayectoria recta','PuntoInicial','PuntoObjetivo','Trayectoria Seguida')
grid minor;
grid on;
hold off
